/**
 * MediCore HMS – Panel Administrador
 * js/panel_admin.js
 */
...
function navigateTo(section) {
  ...
  // Update topbar title
  const titles = {
    dashboard: 'Dashboard', notificaciones: 'Notificaciones', empleados: 'Gestión de Empleados',
    precios: 'Validación de Precios', pagos: 'Gestión de Pagos', mantenimiento: 'Mantenimiento',
    reportes: 'Reportes', auditoria: 'Bitácora de Auditoría'
  };
  document.getElementById('topbarTitle').textContent = titles[section] || section;

  // Lazy load section data — empleados carga sin filtros al entrar
  const loaders = {
...
async function loadDashboard() {
  try {
    ...
    // KPIs
    document.getElementById('kpiEmpleados').textContent  = d.kpis.total_empleados   ?? '–';
    document.getElementById('kpiPacientes').textContent  = d.kpis.total_pacientes   ?? '–';
    document.getElementById('kpiCitasHoy').textContent   = d.kpis.citas_hoy         ?? '–';
    document.getElementById('kpiPagosPend').textContent  = d.kpis.pagos_pendientes  ?? '–';
    document.getElementById('kpiPreciosPend').textContent= d.kpis.precios_pendientes?? '–';
    document.getElementById('kpiStock').textContent      = d.kpis.alertas_stock     ?? '–';
...
function buildChartConsultas(data, resumen) {
  ...
  // Colores: azul para General, escala cálida para Especialistas
        ...
        tooltip: {
          callbacks: {
            label: (ctx) => {
              ...
              return ` ${ctx.label}: ${ctx.parsed} consultas (${pct}%) — ${cat}`;
            }
          }
        },
        // Texto central en el doughnut
        afterDraw: undefined,
      }
    },
    plugins: [{
...
async function loadRolesSelect() {
  try {
    ...
    (consResp.consultorios || []).forEach(c => {
      selCon.innerHTML += `<option value="${c.id_consultorio}">${c.area} – ${c.numero_consultorio}</option>`;
    });
  } catch(e) {
  ...
  try {
    ...
    tbody.innerHTML = d.empleados.map(e => `
      <tr>
        ...
        <td>${e.consultorio_area ? `${e.consultorio_area} – ${e.numero_consultorio}` : '—'}</td>
        <td>${estatusBadge(e.estatus)}</td>
        <td>${e.fecha_baja_programada ? fmtDate(e.fecha_baja_programada) : '—'}</td>
...
async function guardarEmpleado() {
  ...
  if (!id && !pw) return showToast('La contraseña es requerida para nuevos empleados', 'warning');
  if (pw && pw.length < 8) return showToast('La contraseña debe tener al menos 8 caracteres', 'warning');
  if (pw && pw !== pwConf) return showToast('Las contraseñas no coinciden', 'warning');
...
async function confirmarBaja() {
  ...
  try {
    ...
    if (d.error) return showToast(d.error, 'danger');
    showToast('Período de gracia iniciado. Baja definitiva en 30 días.', 'warning');
...
async function openReasignar(id) {
  ...
  document.getElementById('reasPacientesList').innerHTML = '<p class="text-muted">Cargando…</p>';
  document.getElementById('reasReceptor').innerHTML = '<option value="">Cargando…</option>';
  ...
  try {
    ...
    if (items.length === 0) {
      list.innerHTML = '<p class="text-success"><i class="fas fa-check me-1"></i>Sin pacientes pendientes de reasignación</p>';
    } else {
      list.innerHTML = items.map(p => `
        <div class="paciente-item">
          <i class="fas fa-user me-2 text-muted"></i>
          <span>${p.paciente_nombre || p.nombre_paciente || 'Paciente'}</span>
          <span class="ms-auto text-muted small">${p.tipo || 'Cita'} – ${fmtDate(p.fecha_hora || p.fecha_ingreso)}</span>
        </div>
      `).join('');
    }
  } catch(e) {
    showToast('Error al cargar datos de reasignación', 'danger');
  }
}
...
async function bajaDefinitiva(id, nombre) {
  if (!confirm(`¿Confirma la BAJA DEFINITIVA de ${nombre}? Esta acción no se puede revertir.`)) return;
  try {
...
async function loadPrecios() {
  try {
    ...
    if (!d.lotes || d.lotes.length === 0) {
      tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-4">Sin lotes pendientes de validación</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = d.lotes.map(l => `
      <tr>
        ...
        <td>${l.numero_lote}<br><small class="text-muted">Cad: ${l.fecha_caducidad||'—'}</small></td>
        <td>${l.farmaceutico||'—'}</td>
        <td>${l.precio_vigente ? fmt(l.precio_vigente) : '—'}</td>
...
function openValidarPrecio(loteId, med, precio, accion) {
  ...
  document.getElementById('btnConfirmarPrecio').textContent = accion === 'aprobar' ? '✓ Aprobar' : '✗ Rechazar';
  ...
  try {
    ...
    tbody.innerHTML = d.map(m => `
      <tr>
        ...
        <td><strong>${m.folio_pago||'—'}</strong><br><small class="text-muted">${fmt(m.monto_total)}</small></td>
        <td>${m.paciente||'—'}</td>
...
async function accionPago(id, accion) {
  ...
  if (!confirm(`¿Confirma ${label} la modificación #${id}?`)) return;
  try {
    const d = await apiPost('php/api/pagos.php', { action: accion, id_modificacion: id });
    if (d.error) return showToast(d.error, 'danger');
    showToast(accion === 'aprobar' ? 'Modificación autorizada' : 'Modificación rechazada');
    loadPagos();
    loadNotificaciones();
  } catch(e) {
    showToast('Error al procesar la modificación', 'danger');
  }
}
...
async function crearMantenimiento() {
  ...
  if (duracion > 15 || duracion < 1) return showToast('La duración debe ser entre 1 y 15 minutos', 'warning');
  ...
  try {
    ...
    if (d.error) return showToast(d.error, 'danger');
    showToast('Mantenimiento programado. Alertas enviadas a 5 áreas.', 'info');
...
async function cancelarMantenimiento(id) {
  if (!confirm(`¿Cancelar el mantenimiento #${id}?`)) return;
  try {
...
async function loadReporte() {
  ...
  cont.innerHTML = `<div class="text-center py-5 text-muted"><i class="fas fa-spinner fa-spin fa-2x mb-3"></i><p>Cargando reporte…</p></div>`;
...
/** Mensaje estándar de "sin datos" para insertar en un contenedor de gráfica */
function sinDatosHTML(msg='Sin datos para el período seleccionado') {
...
function renderReporteFinanciero(d, cont) {
  ...
  cont.innerHTML = `
    ...
    <div class="charts-grid">
      <div class="chart-card wide"><div class="chart-header"><h3 class="chart-title">Ingresos históricos (12 meses)</h3></div>
...
function renderReporteCliniko(d, cont) {
  ...
  const tablaRows = hayDatos
    ...
    : `<tr><td colspan="3" class="text-center text-muted py-3">Sin consultas en este período</td></tr>`;
  ...
  cont.innerHTML = `
    <div class="charts-grid mb-4">
      ...
      <div class="chart-card wide">
        <div class="chart-header"><h3 class="chart-title">Top diagnósticos del mes</h3></div>
    ...
    <h5 class="subsection-title">Detalle por médico</h5>
    <div class="table-container">
      <table class="data-table"><thead><tr><th>Médico</th><th>Especialidad</th><th>Consultas</th></tr></thead>
  ...
  setTimeout(() => {
    ...
    if (diagnosticos.length > 0) {
      ...
      chartsCreated.rptChartDiag = new Chart(document.getElementById('rptChartDiag').getContext('2d'), {
        type:'bar',
        data:{
          labels: diagnosticos.map(x => x.diagnostico.length > 40 ? x.diagnostico.substring(0,38)+'…' : x.diagnostico),
...
function renderReporteCitas(d, cont) {
  ...
  const tablaRows = porMedico.length > 0
    ...
    : `<tr><td colspan="3" class="text-center text-muted py-3">Sin citas en este período</td></tr>`;
  ...
  cont.innerHTML = `
    <div class="charts-grid mb-4">
      ...
      <div class="chart-card wide">
        <div class="chart-header"><h3 class="chart-title">Citas por médico</h3></div>
        <div class="table-container">
          <table class="data-table"><thead><tr><th>Médico</th><th>Especialidad</th><th>Total citas</th></tr></thead>
          <tbody>${tablaRows}</tbody></table>
        </div>
      </div>
    </div>
...
function renderReporteFarmacia(d, cont) {
  cont.innerHTML = `
    <h5 class="subsection-title">Medicamentos por caducar (&lt; 90 días)</h5>
    <div class="table-container">
      <table class="data-table"><thead><tr><th>Medicamento</th><th>Lote</th><th>Existencia</th><th>Caducidad</th><th>Días restantes</th></tr></thead>
    ...
    <h5 class="subsection-title mt-4">Top 10 más utilizados</h5>
    <div class="charts-grid">
      <div class="chart-card wide">
...
function renderReporteInventario(d, cont) {
  cont.innerHTML = `
    ...
    <div class="table-container">
      <table class="data-table"><thead><tr><th>Artículo</th><th>Categoría</th><th>Área</th><th>Stock actual</th><th>Stock mínimo</th><th>Estado</th></tr></thead>
      <tbody>${(d.insumos||[]).map(i=>`<tr><td>${i.nombre_articulo}</td><td>${i.categoria||'—'}</td><td>${i.area_asignada||'—'}</td><td>${i.stock_actual}</td><td>${i.stock_minimo}</td><td>${estatusBadge(i.estado)}</td></tr>`).join('')}</tbody></table>
    </div>
    <h5 class="subsection-title mt-4">Equipos médicos – revisiones</h5>
    <div class="table-container">
      <table class="data-table"><thead><tr><th>Equipo</th><th>Estatus</th><th>Últ. mantenimiento</th><th>Próxima revisión</th><th>Días</th></tr></thead>
      <tbody>${(d.equipos||[]).map(e=>`<tr class="${e.dias_para_revision<7?'row-danger':e.dias_para_revision<30?'row-warning':''}"><td>${e.nombre_articulo}</td><td>${e.estatus_equipo||'—'}</td><td>${e.fecha_ultimo_mantenimiento||'—'}</td><td>${e.proxima_revision||'—'}</td><td><strong>${e.dias_para_revision??'—'}</strong></td></tr>`).join('')}</tbody></table>
    </div>
  `;
}
  ...
  try {
    ...
    tbody.innerHTML = d.logs.map(l => `
      <tr>
        ...
        <td>${l.usuario_nombre||'—'}<br><small class="text-muted">${l.usuario_tipo||''}</small></td>
        <td><small>${l.nombre_rol||'—'}</small></td>
        <td><span class="badge bg-secondary">${l.modulo}</span></td>
        <td>${l.accion}</td>
        <td><small>${l.ip_equipo||'—'}</small></td>
        <td><small class="text-muted">${l.detalle_cambio ? l.detalle_cambio.substring(0,80)+'…' : '—'}</small></td>
      </tr>
    `).join('');
...
function renderPagination(containerId, current, total, callback) {
  ...
  html += `<li class="${current<=1?'disabled':''}"><a href="#" onclick="return false;" data-p="${current-1}">‹</a></li>`;

  for (let i = Math.max(1,current-2); i <= Math.min(total,current+2); i++) {
    html += `<li class="${i===current?'active':''}"><a href="#" onclick="return false;" data-p="${i}">${i}</a></li>`;
  }

  html += `<li class="${current>=total?'disabled':''}"><a href="#" onclick="return false;" data-p="${current+1}">›</a></li>`;
  html += '</ul></nav>';
  cont.innerHTML = html;
